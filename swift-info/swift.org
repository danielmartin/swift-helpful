#+TITLE: The Swift Programming Language Guide (Swift 5.2)
...
When you use Xcode 11.4 to build Swift 4 and Swift 4.2 code, most Swift
5.2 functionality is available. That said, the following changes are
available only to code that uses Swift 5.2 or later:
...
A target written in Swift 5.2 can depend on a target that's written in
...
code from Swift 4 to Swift 5.2 one framework at a time.
...
they can't use in-out parameters.
